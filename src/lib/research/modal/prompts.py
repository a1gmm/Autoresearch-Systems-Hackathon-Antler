...
RESEARCH_SKILL_PROMPT = (
    ...
    "4. extract_threshold — submit the grounded finding. Be decisive: as SOON as a fetched "
    "source contains the governing clause, call extract_threshold — do NOT keep searching for "
    "a 'better' source. The verbatim_quote MUST be copied exactly from the fetched source text. "
    "If after your fetches you cannot ground it, submit applies=needs_review with an empty "
    "verbatim_quote. Never assert a determination you could not ground in a fetched primary "
    "source. You may only use the tools you are given."
)
