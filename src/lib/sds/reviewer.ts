...
export function reviewSdsDocument(document: SdsDocument): SdsReview {
  ...
  const qualityFindings = buildQualityFindings(document, sectionMap);
...
function reviewUnreadableSdsDocument(document: SdsDocument): SdsReview {
  return {
    ...
    quality_findings: [
      {
        id: `${document.id}_quality_unreadable_text`,
        severity: "critical",
...
function buildQualityFindings(document: SdsDocument, sectionMap: SdsSectionMap): SdsFinding[] {
  ...
  if (missingSections.length > 0) {
    findings.push({
      id: `${document.id}_quality_missing_sections`,
      severity: "critical",
      category: "section_completeness",
    ...
    findings.push({
      id: `${document.id}_quality_all_sections_found`,
      severity: "info",
      category: "section_completeness",
  ...
  if (ambiguousOrMergedSections.length > 0) {
    findings.push({
      id: `${document.id}_quality_ambiguous_or_merged_sections`,
      severity: "warning",
      category: "section_completeness",
  ...
  if (!/\b(product\s*(identifier|name)|trade\s*name|chemical\s*name)\b/i.test(section1Text)) {
    findings.push({
      id: `${document.id}_quality_product_identity`,
      severity: "warning",
  ...
  if (!hasSupplier || !hasEmergencyContact) {
    findings.push({
      id: `${document.id}_quality_supplier_emergency_contact`,
      severity: "warning",
      category: "identity_contact",
  ...
  if (revisionDates.length === 0) {
    findings.push({
      id: `${document.id}_quality_revision_date_missing`,
      severity: "warning",
      category: "freshness",
      title: "Revision date not found",
      reason: "No revision, prepared, or issue date could be identified in the SDS text.",
    });
  } else if (isStaleRevisionDate(latestDate(revisionDates))) {
    findings.push({
      id: `${document.id}_quality_revision_date_stale`,
      severity: "warning",
      category: "freshness",
...
function buildSafetyFindings(document: SdsDocument, sectionMap: SdsSectionMap): SdsFinding[] {
  ...
  for (const rule of SAFETY_RULES) {
    ...
    findings.push({
      id: `${document.id}_safety_${findings.length + 1}`,
...
function determineOverallStatus(
  ...
  if (qualityFindings.some((finding) => finding.title === "Missing SDS sections")) {
    return "incomplete";
  }

  if (qualityFindings.some((finding) => finding.title === "SDS revision date appears stale")) {
    return "stale";
  }

  if (
    qualityFindings.some((finding) => finding.title === "Ambiguous or merged SDS sections") ||
...
function isStaleRevisionDate(date: Date): boolean {
